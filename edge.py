...
def sobel_edge_detection(image, treshold=10):
    ...
    # Apply Sobel filter using cv2.filter2D (the filters)
    gradient_x_filtered = cv2.filter2D(image, -1, MASK_SOBEL['GX'])   # -1 the same type of the input image , it will be the same for the output
   ...
   # Apply nested loops to store edges in the black images
    ...
    for i in range(1, height - 1):
        for j in range(1, width - 1):
            ...
            gy = gradient_y_filtered[i, j]   # apply filters on each pixel

            gradient_x[i, j] = gx   # add the result to the black image
            gradient_y[i, j] = gy
            gradient_magnitude[i, j] = np.sqrt(gx**2 + gy**2)   #Pythagoras theorem (The overall edge strength at a pixel)
...
def kirsch_edge_detection(image):
    ...
    max_direction = np.max(list(filters.values()), axis=0)    # Find the Maximum Response Across All Directions (strongest edge response)

    max_direction = cv2.normalize(
        max_direction, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)  # normalized it

    direction = max(filters, key=lambda k: np.max(filters[k]))   #('N', 'NW', 'W', etc.).

    return max_direction, direction


#  calculates the difference between a central pixel and its neighbors.
...
def contrast_edge_detection(image):
    ...
    smoothed_image += 1e-20    #avoid division by zero
    ...
    return np.clip(contrast_edge_normalized, 0, 255).astype(np.uint8)     # ensure pixels between 0 and 255


#how much the pixel intensities in a local region deviate from their mean
def variance_edge_detection(image):
    ...
    for i in range(1, height - 1):
        for j in range(1, width - 1):
            ...
            variance_edge[i, j] = variance_value  # increase variance = stronger edge
...
def range_edge_detection(image):
    ...
    for i in range(1, height - 1):
        for j in range(1, width - 1):
            neighborhood = image[i-1:i+2, j-1:j+2]
            range_value = np.max(neighborhood) - np.min(neighborhood)  # difference between the max and min pixel values
