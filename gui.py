...
def plt_to_pil(data, title):
    ...
    plt.bar(range(256), data, color='black', width=1)
    plt.title(title)
...
class ImageProcessingApp:
    ...
    def reset_frame(self):
        ...
        self.main_frame = tk.Frame(self.frame,bg="white")
    ...
    def create_top_frame(self):
        ...
        self.next_button = tk.Button(
            self.top_frame, text="Next",bg="lightblue",fg="black", state="disabled")
        self.next_button.pack(side=tk.RIGHT, padx=5, pady=5)
        self.previous_button = tk.Button(
            self.top_frame, text="Previous",bg="lightblue",fg="black", state="disabled")
        self.previous_button.pack(side=tk.LEFT, padx=5, pady=5)

    def window_one(self):
        self.reset_frame()
        tk.Button(self.main_frame, text="Upload Image",
                  command=self.upload_image,bg="lightblue",fg="black").pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Show Image",bg="lightblue",fg="black",
    ...
    def window_two(self):
        ...
        for key, value in ImageProcessingApp.buttons.items():
            tk.Button(self.main_frame, text=key,bg="lightblue",fg="black", command=lambda value=value: self.action(
    ...
    def image_window(self):
        if self.new_image_window is None:
            ...
            self.new_image_window_left = tk.Frame(self.new_image_window,bg="white")
            self.new_image_window_right = tk.Frame(self.new_image_window,bg="white")
            ...
            self.left_single_button = tk.Button(
                self.new_image_window_left, text="Use output",bg="lightblue",fg="black", command=self.use_output)
            self.left_single_button.pack(padx=10, pady=10)

            self.right_previous_button = tk.Button(
                self.new_image_window_right,bg="lightblue",fg="black", text="Previous", state="disabled", command=lambda: self.change_index(False))
            self.right_previous_button.pack(side=tk.LEFT, padx=30, pady=10)
            self.right_next_button = tk.Button(
                self.new_image_window_right,bg="lightblue",fg="black", text="next", state="disabled", command=self.change_index)
    ...
    def edge_window(self):
        ...
        tk.Button(self.main_frame, text="Sobel",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            sobel_edge_detection, "Sobel", ["threshold"])).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Prewitt",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            prewitt_edge_detection, "Prewitt", ["threshold"])).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Range",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            range_edge_detection, "Range")).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Contrast",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            contrast_edge_detection, "Contrast")).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Variance",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            variance_edge_detection, "Variance")).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Difference",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            difference_edge_detection, "Difference", ["threshold"])).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Homogeneity",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            homogeneity_edge_detection, "Homogeneity", ["threshold"])).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Kirsch",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            kirsch_edge_detection, "Kirsch")).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Difference of gaussians",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
    ...
    def segmentation_window(self):
        self.reset_frame()
        self.previous_button.config(state="normal", command=self.window_two)
        tk.Button(self.main_frame, text="Manual",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            manual_segmentation, "Manual", ["low_t", "high_t"])).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Histogram peak",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            histogram_peak_segmentation, "Histogram peak")).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Histogram valley",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            histogram_valley_segmentation, "Histogram valley")).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Adaptive", bg="lightblue",fg="black",command=lambda: self.operate_on_image(
            adaptive_histogram_segmentation, "Adaptive")).pack(fill=tk.X, padx=10, pady=10)

    def halftone_window(self):
        self.reset_frame()
        self.previous_button.config(state="normal", command=self.window_two)
        tk.Button(self.main_frame, text="Simple", bg="lightblue",fg="black",command=lambda: self.operate_on_image(
            Simple_halftone, "Simple", ["threshold"])).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="halftone",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            Halftone, "Halftone")).pack(fill=tk.X, padx=10, pady=10)

    def filter_window(self):
        self.reset_frame()
        self.previous_button.config(state="normal", command=self.window_two)
        tk.Button(self.main_frame, text="low pass",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            lowpass, "low pass")).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="high pass",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            highpass, "high pass")).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="median pass",bg="lightblue",fg="black", command=lambda: self.operate_on_image(
            median_filter, "median pass")).pack(fill=tk.X, padx=10, pady=10)

    def operations_window(self):
        self.reset_frame()
        self.previous_button.config(state="normal", command=self.window_two)
        tk.Button(self.main_frame, text="Add image",bg="lightblue",fg="black", command=lambda: self.image_operations(
            add, "Add image")).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Subtract image", bg="lightblue",fg="black",command=lambda: self.image_operations(
            sub, "Subtract image")).pack(fill=tk.X, padx=10, pady=10)
        tk.Button(self.main_frame, text="Invert image",bg="lightblue",fg="black", command=lambda: self.image_operations(
