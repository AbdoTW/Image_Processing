...
# highlight pixels between range
...
def histogram_peak_segmentation(image):
    ...
    peaks_indices = find_histogram_peaks(histogram)    # local maximum, representing a value that occurs frequently in the image.
    low_T, high_T = calculate_thresholds(peaks_indices, histogram)  # calculate lower and higher threshold

    segmented_image_peak = np.zeros_like(image)
    segmented_image_peak[(image >= low_T) & (image <= high_T)] = 255   #region between these thresholds is highlighted
...
# represents a region with fewer pixel intensities, typically occurring between peaks (intensity clusters).
# The function identifies these valleys and uses them to define thresholds for segmentation.
...
#top two peaks in a histogram.
def find_histogram_peaks(histogram):
    peaks, _ = find_peaks(histogram, height=0)
    sorted_peaks = sorted(peaks, key=lambda x: histogram[x], reverse=True)
    return sorted_peaks[:2]

# valley point (the minimum intensity) between two peaks in the histogram.
...
# low and high thresholds for segmentation
...
def adaptive_histogram_segmentation(image):
    ...
    background_mean, object_mean = calculate_means(segmented_image, image)  # background and object mean
    new_peaks_indices = [int(background_mean), int(object_mean)]
    new_low_T, new_high_T = valley_high_low(new_peaks_indices, find_valley_point(   # low and high thresholds for segmentation with new peaks
        new_peaks_indices, histogram ))
